...
export const Emporios = () => {
  ...
  return (
    <section className='flex flex-col justify-center items-center my-3'>
      <h1 className='text-2xl md:text-5xl uppercase text-branco-quente font-bold my-1 md:my-5'>Emporios</h1>
      {loading ? (
        <Loader />
      ) : (
        <article className='sm:grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 md:px-[30px] lg:px-[100px]'>
          {emporios.map(emporios => (
            <div key={emporios.id} className='bg-branco-quente border border-marrom-claro rounded p-4 m-4'>
              ...
              <a className='flex items-center gap-2 mt-2' href={emporios.url}>Instagram <i className="fa-brands fa-instagram text-2xl"></i></a>
            </div>
          ))}
        </article>
      )}
