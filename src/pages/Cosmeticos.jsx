      ...
      {loading ? (
        ...
        <article className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-[1400px]'>
          {cosmeticos.map(cosmetico => (
            <div key={cosmetico.id} className='bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full'>
              <div className='p-6 flex-grow'>
                ...
                {cosmetico.endereco && (
                  <div className='flex items-start gap-2 text-xs text-gray-500 bg-gray-50 rounded-lg p-3'>
                    ...
                    <span className='font-medium'>{cosmetico.endereco}</span>
                  </div>
                )}
              </div>
