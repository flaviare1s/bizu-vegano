      ...
      {loading ? (
        ...
        <article className='sm:grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 md:px-[30px] lg:px-[100px]'>
          {restaurantes.map(restaurante => (
            <div key={restaurante.id} className='bg-branco-quente border border-marrom-claro rounded p-4 m-4 relative'>
              ...
              <a className='flex items-center gap-2 mt-2 absolute bottom-2 right-3' href={restaurante.url}><i className="fa-brands fa-instagram text-2xl"></i></a>
            </div>
          ))}
        </article>
      )}
