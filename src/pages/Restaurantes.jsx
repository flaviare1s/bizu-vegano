  ...
  return (
    <section className='flex flex-col justify-center items-center my-6 px-4'>
      <div className='border-b-2 border-logo w-full mb-5'>
        <h1 className='text-2xl text-center md:text-4xl text-verde-escuro font-bold my-4 md:my-6'>Restaurantes</h1>
      </div>
      {loading ? (
        <Loader />
      ) : (
        <article className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-[1400px]'>
          {restaurantes.map(restaurante => (
            <div key={restaurante.id} className='bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full border-l-4 border-logo'>
              <div className='p-6 flex-grow'>
                <div className='flex items-center gap-4 mb-4'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <h3 className='font-bold text-lg text-gray-800 leading-tight mb-1'>{restaurante.nome}</h3>
                  </div>
                </div>

                <p className='text-gray-600 text-sm leading-relaxed mb-4 font-semibold'>{restaurante.descricao}</p>
                ...
                {restaurante.endereco ? (
                  ...
                  >
                    ...
                    <span className='font-medium hover:text-verde-oliva transition-colors'>{restaurante.endereco}</span>
                  </a>
                ) : (
