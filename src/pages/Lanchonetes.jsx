  ...
  return (
    <section className='flex flex-col justify-center items-center my-6 px-4'>
      <div className='border-b-2 border-logo w-full mb-5'>
        <h1 className='text-2xl text-center md:text-4xl text-verde-escuro font-bold my-4 md:my-6'>Lanchonetes</h1>
      </div>
      {loading ? (
        <Loader />
      ) : (
        <article className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-[1400px]'>
          {lanchonetes.map(lanchonete => (
            <div key={lanchonete.id} className='bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full border-l-4 border-logo'>
